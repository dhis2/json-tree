...
/**
 * Tests Validation of the @{@link Validation#enumeration()} and @{@link Validation#oneOfValues()}
 * properties.
 *
 * @author Jan Bernitt
 */
class JsonValidationEnumTest {
  ...
  public interface JsonEnumExampleB extends JsonObject {
    ...
    default JsonMixed isOn() {
      return get("on", JsonMixed.class);
    }
  }
    ...
    assertDoesNotThrow(
        () ->
            JsonMixed.of(
                    """
            {"type": "hello"}""")
                .validate(JsonEnumExampleA.class));
    assertDoesNotThrow(
        () ->
            JsonMixed.of(
                    """
    ...
    assertDoesNotThrow(
        () ->
            JsonMixed.of(
                    """
            {"on": 1}""")
                .validate(JsonEnumExampleB.class));
    assertDoesNotThrow(
        () ->
            JsonMixed.of(
                    """
  ...
  void testEnum_OneOfValuesMixed_Invalid() {
    assertValidationError(
        """
            {"on": "1"}""",
        JsonEnumExampleB.class,
        Rule.ENUM,
        Set.of("false", "1", "0", "true", "\"yes\"", "\"no\""),
        "\"1\"");
  }
    ...
    assertDoesNotThrow(
        () ->
            JsonMixed.of(
                    """
            {"type": "TYPE"}""")
                .validate(JsonEnumExampleC.class));
    assertDoesNotThrow(
        () ->
            JsonMixed.of(
                    """
  ...
  void testEnum_Enumeration_Invalid() {
    Set<String> expectedValidValues =
        Set.of(
            "ANNOTATION_TYPE",
            "RECORD_COMPONENT",
            "TYPE_PARAMETER",
    ...
    assertDoesNotThrow(
        () ->
            JsonMixed.of(
                    """
            {"type": "tyPe"}""")
                .validate(JsonEnumExampleD.class));
    assertDoesNotThrow(
        () ->
            JsonMixed.of(
                    """
