...
class JsonValidationMiscDeepGraphTest {
  ...
  void testDeep_Valid_EmptyPage() {
    assertDoesNotThrow(
        () ->
            JsonMixed.of(
                    """
            {"pager": {"size": 20, "page": 0, "total": 0}, "entries": []}""")
                .validate(JsonPage.class));
  }
  ...
  void testDeep_Error_PagerPageNoInteger() {
    Validation.Error error =
        assertValidationError(
            """
                {"pager": {"size": 20, "page": true}, "entries":[]}""",
            JsonPage.class,
            Rule.TYPE,
  ...
  void testDeep_Error_PagerTotalWrongType() {
    Validation.Error error =
        assertValidationError(
            """
                {"pager": {"size": 20, "page": 0, "total": "yes"}, "entries":[]}""",
            JsonPage.class,
            Rule.TYPE,
            Set.of(NodeType.INTEGER),
            NodeType.STRING);
  ...
  void testDeep_Error_NoEntries() {
    ...
    assertValidationError(
        """
            {"pager": {"size": 20, "page": 0}, "entries":null}""",
        JsonPage.class,
        Rule.REQUIRED,
        "entries");
  }
  ...
  void testDeep_Error_IdWrongType() {
    ...
    Validation.Error error =
        assertValidationError(
            json, JsonPage.class, Rule.TYPE, Set.of(NodeType.STRING), NodeType.NUMBER);
    assertEquals(".entries.0.id", error.path().toString());
  }
  ...
  void testDeep_Error_ValuesWrongType() {
    ...
    Validation.Error error =
        assertValidationError(
            json, JsonPage.class, Rule.TYPE, Set.of(NodeType.OBJECT), NodeType.BOOLEAN);
  ...
  void testDeep_Error_AttributesAsMap() {
    ...
    Validation.Error error =
        assertValidationError(
            json, JsonPage.class, Rule.TYPE, Set.of(NodeType.ARRAY), NodeType.OBJECT);
