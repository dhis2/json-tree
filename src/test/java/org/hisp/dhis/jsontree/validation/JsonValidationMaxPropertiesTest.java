...
class JsonValidationMaxPropertiesTest {
    ...
    assertDoesNotThrow(
        () ->
            JsonMixed.of(
                    """
            {"config":{"a": 1, "b": 2}}""")
                .validate(JsonMaxPropertiesExampleA.class));

    assertDoesNotThrow(
        () ->
            JsonMixed.of(
                    """
            {"points":{"zero": 0, "x": 7, "y": 9}}""")
  ...
  void testMaxProperties_TooMany() {
    assertValidationError(
        """
                {"config":{"hey": 1, "ho": 2, "silver": 3}}""",
        JsonMaxPropertiesExampleA.class,
        Rule.MAX_PROPERTIES,
        2,
        3);
    assertValidationError(
        """
                {"points":{"x": 1, "y":  2, "z": 3, "w": 5}}""",
        JsonMaxPropertiesExampleB.class,
        Rule.MAX_PROPERTIES,
        3,
        4);
  }
