...
public @interface TerminalOp {
   ...
   *     least treats empty objects as empty arrays. When used with {@link #canBeUndefined()} JSON
   *     null and undefined is also permitted. When used with {@link #canBeNull()} JSON null is
   *     permitted.
   */
  boolean mustBeArray() default false;

  /**
   * @return when true, the node must be an object. When used with {@link #mustBeArray()} arrays are
   *     also permitted. When used with {@link #canBeUndefined()} JSON null and undefined is also
   *     permitted. When used with {@link #canBeNull()} JSON null is permitted.
   */
