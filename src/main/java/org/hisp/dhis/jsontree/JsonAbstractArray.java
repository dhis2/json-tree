...
public interface JsonAbstractArray<E extends JsonValue>
    extends JsonAbstractCollection, Iterable<E> {

  /**
   * A typed variant of {@link JsonArray#get(int)}, equivalent to {@link JsonArray#get(int, Class)}
   * where 2nd parameter is the type parameter E.
   *
  ...
  /**
   * True, if this list contains all the values provided given each value of the list is transformed
   * by the toValue function to access its comparable value.
   *
   ...
   *
   * <p>If no such element is returned a non-existing element at index of size of this list is
   * returned.
   *
