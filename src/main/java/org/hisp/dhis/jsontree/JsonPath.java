 ...
 *
 * <p>When constructed from a user input {@link String} in {@link #of(CharSequence)} the provided
 * string might require splitting into segments. Therefore, there is a syntax to indicate segments.
 * The most common form is the {@code .}-syntax where dot marks the start of a segment. The initial
 * segment can omit the leading dot.
 *
 ...
 * @author Jan Bernitt
 * @since 1.1
 * @param parent null in case there is no parent (1st segment does not explicitly point to empty
 *     path)
...
public record JsonPath(@CheckNull JsonPath parent, @CheckNull Text segment)
  ...
  public static JsonPath of(CharSequence... segments) {
    ...
    for (int i = 1; i < segments.length; i++) res = res.chain(Text.of(segments[i]));
    return res;
  }
   ...
   * @return a new path instance that starts with all segments of this path followed by all segments
   *     of the provided sub-path
   */
  public JsonPath concat(JsonPath subPath) {
    ...
    JsonPath res = this;
    for (Text segment : subPath.segments()) res = new JsonPath(res, segment);
    return res;
  }
   ...
   * @return a new path instance that adds the provided object member name segment to this path to
   *     create a new absolute path for the same root
   */
   ...
   * @return a new path instance that adds the provided array index segment to this path to create a
   *     new absolute path for the same root
   */
  public JsonPath chain(int index) {
    if (index < 0)
      throw new JsonPathException(
          this, "Path array index must be zero or positive but was: %d".formatted(index));
  ...
  @Override
  public int compareTo(@NotNull JsonPath b) {
    JsonPath a = this;
  ...
  private static String escape(Text seg, JsonPath context) throws JsonPathException {
    ...
    // common special case: has a square but no curly or dot => only needs escaping when open +
    // close square
  ...
  /**
   * Searches for the end since possibly a curly escape is used and a valid inner curly end would be
   * misunderstood.
   */
   ...
   * @return splits the path into segments each starting with a character that {@link
   *     #isSyntaxIndicator(char)}
   * @throws JsonPathException when the path cannot be split into segments as it is not a valid path
   */
  private static JsonPath splitPath(JsonPath parent, Text path) throws JsonPathException {
    ...
    while (i < len) {
      ...
      if (c == '[' && isSquareSegmentOpen(path, i)) {
        ...
        throw new JsonPathException(
            cur, "Malformed path %s, invalid start of segment at position %d.".formatted(path, i));
  ...
  private static boolean isDotSegmentClose(Text path, int offset) {
    return path.charAt(offset) == '.'
        || isCurlySegmentOpen(path, offset)
        || isSquareSegmentOpen(path, offset);
  }
  ...
  private static boolean isCurlySegmentClose(Text path, int offset) {
    return path.charAt(offset) == '}'
        && (offset + 1 >= path.length() || isSyntaxIndicator(path.charAt(offset + 1)));
  }
