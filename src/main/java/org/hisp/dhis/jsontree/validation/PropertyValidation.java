...
record PropertyValidation(
    ...
    PropertyValidation withCustoms(@Surly List<Validator> validators) {
        ...
        ValueValidation newValues = values == null
            ? new ValueValidation( YesNo.AUTO, Set.of(), validators )
            : new ValueValidation( values.required, values.anyOfJsons, validators );
     ...
     * @param customs a validator defined by class is used (custom or user defined validators), empty list is off
     */
    record ValueValidation(@Surly YesNo required, @Surly Set<String> anyOfJsons, @Surly List<Validator> customs) {
    ...
    record StringValidation(@SuppressWarnings( "rawtypes" )
                            @Surly Class<? extends Enum> anyOfNames, int minLength, int maxLength, @Surly String pattern) {
     ...
     */
    record NumberValidation(double minimum, double maximum, double exclusiveMinimum, double exclusiveMaximum, double multipleOf) {
