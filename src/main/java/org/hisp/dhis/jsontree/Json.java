 ...
 *
 * <p>It builds on top of the individual APIs like {@link JsonValue}, {@link JsonMixed} and {@link
 * JsonBuilder}.
 *
 * <p>In contrast to the lower level APIs the {@link Json} API accepts Java values as input. For
 * example, a {@link String} is understood not as a JSON string value with quotes but a Java string
 * value that is escaped to create the JSON string equivalent.
 *
 * @author Jan Bernitt
 * @since 0.11
 */
public interface Json {
  ...
  static <E> JsonArray array(Function<E, ? extends JsonValue> map, Iterable<E> items) {
    return items == null
        ? ofNull()
        : array(arr -> items.forEach(e -> arr.addElement(map.apply(e).node())));
  }
  ...
  static <T> JsonObject object(
      Function<T, ? extends JsonValue> map, Iterable<Map.Entry<String, T>> items) {
    return items == null
        ? ofNull()
        : object(
            obj -> items.forEach(e -> obj.addMember(e.getKey(), map.apply(e.getValue()).node())));
  }

  static <T> JsonObject object(
      Function<T, ? extends JsonValue> map, Stream<Map.Entry<String, T>> items) {
    return object(
        obj -> items.forEach(e -> obj.addMember(e.getKey(), map.apply(e.getValue()).node())));
  }
